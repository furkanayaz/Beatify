...
class ArtistDetailVM @Inject constructor(
    ...
    fun fetchData(artistId: Int) {
        _albumModel.postValue(BeatifyResponse.Loading())

        viewModelScope.launch {
            _connObserver.collect {
                when(it) {
                    Connection.Status.Available -> getAlbums(artistId = artistId)
                    Connection.Status.Losing -> _albumModel.postValue(BeatifyResponse.Loading())
                    Connection.Status.Unavailable, Connection.Status.Lost -> _albumModel.postValue(BeatifyResponse.Failure(code = 404))
                }
            }
        }
    }
